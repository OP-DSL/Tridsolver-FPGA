...
static void read_u(const uint512_dt* u, hls::stream<uint256_dt> &u_stm,
	...
	for(ap_uint<12> i = 0; i < B; i++){
		#pragma HLS loop_tripcount min=100 max=200 avg=200
		for(ap_uint<12> j = 0; j < N; j++){
			#pragma HLS loop_tripcount min=256 max=512 avg=512
			int offset = j* (((B*(M>>3)+3)>>2)<< 2) + i * (M>>3);
			for(ap_uint<12> k = 0; k < (M >> 3); k++){
				#pragma HLS PIPELINE II=2
				#pragma HLS loop_tripcount min=32 max=64 avg=64
				uint512_dt tmp = u[offset + k];
				uint512_dt tmp_acc = acc[offset + k];
...
//	printf("read_u: I didn't get stuck here\n");
}
	...
	for(ap_uint<12> i = 0; i < B; i++){
		...
		for(ap_uint<12> j = 0; j < N; j++){
			...
			for(ap_uint<12> k = 0; k < (M >> 3); k++){
				...
				d[offset+k] = tmp_d;
				acc[offset+k] = tmp_acc;
			}
		}
	}
//	printf("write_d: I didn't get stuck here\n");
}
...
static void process_grid( hls::stream<uint256_dt> &rd_buffer, hls::stream<uint256_dt> &wr_buffer,
	...
	double row_arr3[VEC_FACTOR];
	double row_arr2[VEC_FACTOR + 2];
	double row_arr1[VEC_FACTOR];
	double mem_wr[VEC_FACTOR], acc_out_arr[VEC_FACTOR];
	double a_arr[VEC_FACTOR], b_arr[VEC_FACTOR], c_arr[VEC_FACTOR];
	double acc_in_arr[VEC_FACTOR];
...
//	#pragma HLS RESOURCE variable=row3_n core=XPM_MEMORY uram latency=2
    ...
    for(unsigned int itr = 0; itr < grid_size; itr++) {
        #pragma HLS loop_tripcount min=1638400 max=2038400 avg=2038400
...
//        	rd_in_vec
        ...
        process: for(short q = 0; q < VEC_FACTOR; q++){
            ...
            double r1 = ( (row_arr2[q])  + (row_arr2[q+2]) );

            double r2 = ( row_arr1[q]  + row_arr3[q] );

            double f1 = r1 + r2;
            double result  = f1 - row_arr2[q+1]*6.0;
            bool change_cond = (index <= 0 || index > sizex || (i == 1) || (i == end_row));
            mem_wr[q] = change_cond ? 0.0f : result;

            a_arr[q] = change_cond ? 0.0f : -0.5;
            b_arr[q] = change_cond ? 1.0f :  2.0;
            c_arr[q] = change_cond ? 0.0f : -0.5;
...
//    printf("process_grid: I didn't get stuck here\n");
}
...
//---------------------------------
	...
	process_grid(rd_buffer, wr_buffer, acc_in, acc_out, data_g, dnt_acc_updt);
