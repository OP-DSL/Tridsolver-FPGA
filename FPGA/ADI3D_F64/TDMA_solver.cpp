...
static void read_coeff(const uint512_dt*d, hls::stream<uint512_dt> &d_stm,
...
static void stream_convert_512_256(hls::stream<uint512_dt> &in, hls::stream<uint256_dt> &out, unsigned int total_itr){
	for (int itr = 0; itr < total_itr; itr++){
...
static void stream_convert_256_512(hls::stream<uint256_dt> &in, hls::stream<uint512_dt> &out, unsigned int total_itr){
	for (int itr = 0; itr < total_itr; itr++){
		#pragma HLS PIPELINE II=2
		#pragma HLS loop_tripcount min=min_grid max=max_grid avg=avg_grid
		uint512_dt tmp;
...
static void interleaved_row_col(hls::stream<uint256_dt> &in, hls::stream<uint256_dt> &out,
	...
	uint256_dt tmp_M[N_MAX*N_MAX/4*2];
	...
	for(int itr= 0; itr < total_itr; itr++){
		...
		int indW = k*TileY+j;
		int indR = j*TileX + k;
		unsigned int offsetR = ((i & 1) == 0) ?  N_MAX*N_MAX/4 : 0;
		unsigned int offsetW = ((i & 1) == 0) ?  0 : N_MAX*N_MAX/4;
		if(cmp && i < NTiles){
			tmp = in.read();
		}
...
static void write_u(uint512_dt* u, hls::stream<uint512_dt> &u_stm,
		ap_uint<9> M, ap_uint<9> N, ap_uint<9> L, ap_uint<10> B, unsigned char dirXYZ){
...
static void TDMA_pre_XY(const uint512_dt* d, uint512_dt* u,
	...
	read_u(d, data_512_stm[0], acc_1, data_512_stm[1],  M, N, L, B);
	stream_convert_512_256(data_512_stm[0], streamArray[0], total_512_data);
...
//	TDMA_comp(u_stm[3], u_stm[4], M, N, L, B, 1);
	...
	printf("I have processed all\n");

	stream_convert_256_512(u_stm[5], data_512_stm[2], total_512_data);
	stream_convert_256_512(accStream[1], data_512_stm[3], total_512_data);

	write_u(u, data_512_stm[2], M, N, L, B, 0);
	write_u(acc_2, data_512_stm[3], M, N, L, B, 0);
...
static void TDMA_Z(const uint512_dt* d, uint512_dt* u,
	...
	#pragma HLS dataflow
	read_coeff(d, data_512_stm[0], M, N, L, B, dirXYZ);
	stream_convert_512_256(data_512_stm[0], d_stm[0], total_512_data);
...
//	TDMA_comp(d_stm[1], u_stm[0], M, N, L, B, dirXYZ);
	...
	stream_convert_256_512(u_stm[3], data_512_stm[1], total_512_data);
	write_u(u, data_512_stm[1], M, N, L, B, dirXYZ);
