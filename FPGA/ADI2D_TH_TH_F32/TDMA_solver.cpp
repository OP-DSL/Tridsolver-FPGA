...
static void interleaved_row_block8(hls::stream<uint256_dt> &in, hls::stream<uint256_dt> &out,
				unsigned short M, unsigned short N, ap_uint<8> B, unsigned char dirXYZ){
...
static void pcr_solver(hls::stream<float> &ra_stm_in, hls::stream<float> &rb_stm_in,
		hls::stream<float> &rc_stm_in, hls::stream<float> &rd_stm_in,
		hls::stream<float> &ru_stm_out_t, hls::stream<float> &ru_stm_out_b,
		ap_uint<7> n, ap_uint<6> bsize, ap_uint<4> logn, ap_uint<12> bigbatch){
	...
	// copy loop
	for(int bat = 0; bat < bigbatch; bat++){
		for(int i = 0; i < n; i++){
			for(int j = 0; j < bsize; j++){
				inlow[i+j*n] = ra_stm_in.read();
				indiag[i+j*n] = rb_stm_in.read();
				inup[i+j*n] = rc_stm_in.read();
				inrhs[i+j*n] = rd_stm_in.read();
			}
		}

		xf::fintech::trsvCore <float, 512> (inlow, indiag, inup, inrhs, n, bsize, logn);


		for(int j = 0; j < (bsize>>3); j++){
			for(int i = 0; i < n; i++){
				for(int v =0; v< 8; v++){
					int ind = ((j <<3)+v)*n+i;
					if((i & 1) == 0){
						ru_stm_out_t << inrhs[ind]/indiag[ind];
					} else {
						ru_stm_out_b << inrhs[ind]/indiag[ind];
					}
				}
			}
		}
...
static void TT_STAGE4_scalar(hls::stream<float> &ra_stm_in, hls::stream<float> &rb_stm_in, hls::stream<float> &rc_stm_in, hls::stream<float> &rd_stm_in,
	...
	ap_uint<16> TilesN_6 = (ap_uint<16>)Ti << 6;
...
static void TDMA(const uint512_dt* d, uint512_dt* u,
	...
	#pragma HLS STREAM variable = STAGE_2A depth = 510

//	#pragma HLS bind_storage variable=STAGE_2A type=fifo impl=uram

	#pragma HLS STREAM variable = STAGE_2B depth = 510
	...
	#pragma HLS STREAM variable = STAGE_6 depth = 2
	#pragma HLS STREAM variable = STAGE_7 depth = 2

	#pragma HLS STREAM variable = STAGE_2B_scl depth = 2
	#pragma HLS STREAM variable = STAGE_4_scl depth = 512
	#pragma HLS STREAM variable = STAGE_6_scl depth = 512
...
//		printf("finished stage 1\n");
	...
	URAM_buffer<4096>(STAGE_2A[0], STAGE_2A[3],total_data, delay);
	URAM_buffer<4096>(STAGE_2A[1], STAGE_2A[4],total_data, delay);
	URAM_buffer<4096>(STAGE_2A[2], STAGE_2A[5],total_data, delay);

	printf("finished URAM_buffer 2\n");
	...
	vec_to_float(STAGE_2B[3], STAGE_2B_scl[3], R_size);
	vec_to_float(STAGE_2B[4], STAGE_2B_scl[4], R_size);
	vec_to_float(STAGE_2B[5], STAGE_2B_scl[5], R_size);
	vec_to_float(STAGE_2B[6], STAGE_2B_scl[6], R_size);


//		pcr_solver(STAGE_2B_scl[3], STAGE_2B_scl[4], STAGE_2B_scl[5], STAGE_2B_scl[6],
//				STAGE_6_scl[0], STAGE_6_scl[1],
//				Rn, R_systems, logn, B_size);
//		printf("finished PCR solver 2\n");
//
//		printf("finished vec_to_float stage 4\n");

		// Reduced system FW path
	TT_STAGE4_scalar(STAGE_2B_scl[3], STAGE_2B_scl[4], STAGE_2B_scl[5], STAGE_2B_scl[6],
			STAGE_4_scl[0], STAGE_4_scl[1],
			B_size, Tiles, M, R_systems, Ti);

//		TT_STAGE4(STAGE_2B[3], STAGE_2B[4], STAGE_2B[5], STAGE_2B[6],
//						STAGE_4[0], STAGE_4[1],
//						B_size, Tiles, M, Sys, Ti);
//		printf("finished stage 4\n");


		// reduced system BW path
	TT_STAGE6_scalar(STAGE_4_scl[0], STAGE_4_scl[1],
			STAGE_6_scl[0], STAGE_6_scl[1],
			B_size, Tiles, M, R_systems, Ti);
...
//		float_to_vec(STAGE_4_scl[0], STAGE_4[0], R_size);
//		float_to_vec(STAGE_4_scl[1], STAGE_4[1], R_size);



//
//		TT_STAGE6(STAGE_4[0], STAGE_4[1],
//						STAGE_6[0], STAGE_6[1],
//						B_size, Tiles, M, Sys, Ti);
		printf("finished stage 6\n");
//
		float_to_vec(STAGE_6_scl[0], STAGE_6[0], R_size_half);
		float_to_vec(STAGE_6_scl[1], STAGE_6[1], R_size_half);
		printf("finished float_to_vec stage 4\n");

		// back substitution
		TT_STAGE7(STAGE_2A[3], STAGE_2A[4], STAGE_2A[5],
...
//
//sp=TDMA_batch_7.d:HBM[12:13]
//sp=TDMA_batch_7.u:HBM[12:13]
//sp=TDMA_batch_7.acc_1:HBM[12:13]
//sp=TDMA_batch_7.acc_2:HBM[12:13]
//
//sp=TDMA_batch_8.d:HBM[14:15]
//sp=TDMA_batch_8.u:HBM[14:15]
//sp=TDMA_batch_8.acc_1:HBM[14:15]
//sp=TDMA_batch_8.acc_2:HBM[14:15]
...
//		TDMA(/*a, b, c,*/ d, u, M, N, B, 0);
//		TDMA(/*a, b, c,*/ u, d, M, N, B, 1);
