...
//	printf("TDMA_comp:Read Limit is %d\n", ReadLimit);
	...
	TT_Interleave<0, float, 4096>(d_stm[3],
	...
	vec_to_Dtype<0, float>(STAGE_2B[3], STAGE_2B_scl[3], R_size);
	vec_to_Dtype<0, float>(STAGE_2B[4], STAGE_2B_scl[4], R_size);
	vec_to_Dtype<0, float>(STAGE_2B[5], STAGE_2B_scl[5], R_size);
	vec_to_Dtype<0, float>(STAGE_2B[6], STAGE_2B_scl[6], R_size);



		// Reduced system FW path
	TT_ReducedFW_scalar<0, float, 4096>(STAGE_2B_scl[3], STAGE_2B_scl[4], STAGE_2B_scl[5], STAGE_2B_scl[6],
			STAGE_4_scl[0], STAGE_4_scl[1],
			B_size, Tiles, M, R_systems, Ti);


		// reduced system BW path
	TT_ReducedBW_scalar<0, float, 4096>(STAGE_4_scl[0], STAGE_4_scl[1],
			STAGE_6_scl[0], STAGE_6_scl[1],
			B_size, Tiles, M, R_systems, Ti);


	printf("finished stage 6\n");
	Dtype_to_vec<0, float>(STAGE_6_scl[0], STAGE_6[0], R_size_half);
	Dtype_to_vec<0, float>(STAGE_6_scl[1], STAGE_6[1], R_size_half);
	printf("finished float_to_vec stage 4\n");

	// back substitution
	TT_BackSubstitution<0, float, 4096>(STAGE_2A[3], STAGE_2A[4], STAGE_2A[5],
	...
	FIFO_256_512(u_stm[2], data_512_stm[1], total_512_data);
	write_Tile512<0>(u, data_512_stm[1], Xdim, Ydim, Batch, dirXYZ);
